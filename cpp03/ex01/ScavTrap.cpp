...
{
    if (_energy_points == 0)
        std::cout << "[Attack] "<< _name << " cant anything(energy emtpy!)." << std::endl;
    else
    {
        _energy_points--;
...
void ScavTrap::guardGate( void ) {
    if (_hit_points == 0)
        std::cout  << YELLOW << "[ScavTrap][guardGate] " << _name << " is dead, can't be in gate keeper mode." << WHITE << std::endl;
    else
        std::cout  << YELLOW << "[ScavTrap][guardGate] " << _name << " is in gatekeeper mode " << WHITE << std::endl;
}
